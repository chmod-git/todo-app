...
func NewTelegramBotService(acc *AccountService, auth *AuthorizationService, lists *ListService, tasks *TaskService) *TelegramBotService {
...
func (t *TelegramBotService) LaunchBot() {
	...
	for update := range updates {
		go func(update tgbotapi.Update) {
			...
			if update.Message != nil && update.Message.IsCommand() {
				t.HandleCommand(bot, update)
			}

			if update.CallbackQuery != nil {
				t.HandleCallback(bot, update)
			}

			if update.Message != nil && !update.Message.IsCommand() {
				switch session.Status {
				case "sign_in":
					t.auth.HandleSignInMessage(bot, update, session)
				case "sign_up":
					t.auth.HandleSignUpMessage(bot, update, session)
				case "edit_name", "edit_username", "edit_password":
					t.acc.HandleEditAccountMessage(bot, update, session)
				case "add_task_title", "add_task_description":
					t.tasks.HandleAddTaskMessage(bot, update, session)
				case "add_list_title", "add_list_description":
					t.lists.HandleAddListMessage(bot, update, session)
				case "edit_task_title", "edit_task_description":
					t.tasks.HandleUpdateTaskMessage(bot, update, session)
				case "edit_list_title", "edit_list_description":
					t.lists.HandleManageListInfoMessage(bot, update, session)
				}
			}
		}(update)
	}
...
func (t *TelegramBotService) HandleCommand(bot *tgbotapi.BotAPI, update tgbotapi.Update) {
	switch update.Message.Command() {
	case "start":
		SendMessage(bot, update.Message.Chat.ID, "Hi! I'm bot created to help you manage your tasks")
		t.auth.Auth(bot, update)
	case "auth":
		t.auth.Auth(bot, update)
	case "account":
		t.acc.ManageAccount(bot, update)
	case "lists":
		t.lists.Lists(bot, update)
	case "help":
...
func (t *TelegramBotService) HandleCallback(bot *tgbotapi.BotAPI, update tgbotapi.Update) {
	chatID := GetChatID(update)
	session := GetSession(chatID)

	switch update.CallbackQuery.Data {
	case "sign_in":
		t.auth.SignIn(bot, update)
		session.Status = "sign_in"
	case "sign_up":
		t.auth.SignUp(bot, update)
		session.Status = "sign_up"
	case "edit_acc":
		t.acc.EditAcc(bot, update)
	case "edit_name_yes":
	...
	case "log_out":
		t.acc.LogOut(bot, update)
	case "manage_lists":
		t.lists.ManageLists(bot, update, session)
	case "edit_list_tasks":
		t.lists.PromptListSelection(bot, chatID, session, "edit_tasks")
	case "manage_list_info":
		t.lists.PromptListSelection(bot, chatID, session, "manage_info")
	case "delete_list":
		t.lists.PromptListSelection(bot, chatID, session, "delete_list")
	...
	case "edit_list_description_no":
		...
		err = t.lists.redis.UpdateListData(fmt.Sprintf("%d", chatID), session.ListData, 900)
		if err != nil {
		...
		session.Status = ""
	case "add_list":
		t.lists.AddList(bot, chatID, session)
	default:
		if callbackData := strings.Split(update.CallbackQuery.Data, "|"); len(callbackData) == 2 {
			...
			switch action {
			case "edit_tasks":
				t.tasks.EditListTasks(bot, chatID, id, session)
			case "manage_info":
				t.lists.ManageListInfo(bot, chatID, id, session)
			case "delete_list":
				t.lists.DeleteList(bot, chatID, id, session)
			case "add_task", "update_task", "delete_task":
				session.CurrentListID = id
				t.tasks.HandleTaskAction(bot, chatID, session, action, id)
			case "processing_updating_task":
				t.tasks.UpdateTask(bot, chatID, id, session)
			case "processing_deleting_task":
				t.tasks.DeleteTask(bot, chatID, id, session)
			default:
